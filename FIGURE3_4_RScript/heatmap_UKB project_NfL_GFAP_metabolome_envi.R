...
# ------------------------------------------------------------------------------------------- #
# -------------------------------------- #
# Heatmap for NfL/GFAP and air pollution
# -------------------------------------- #
colnames(nfl_gfap_long_final)

colnames(air_pollution_long_final)

heatmap <- function(data, exposure = "Neurodegenerative markers", transition.outcome = "PD"){
  # color for mediation
  ...
  data.long <- data %>% 
    ...
    filter(transition == transition.outcome)
  
  # assign factor level based on the exposure
  ...
  p.plot <- p.plot +
    ...
    geom_point(aes(size=ap, 
                   ...
                   fill=estimate))  +
    theme_minimal() +
    scale_size_manual(values=c("1"= 7, "2"= 5, "3"=3, "4"= 2),
    ...
    scale_shape_manual(values = c('insig'=1,'Pass'=21),
                       breaks = c("insig", "Pass"), drop = FALSE, guide = "none")+
    guides(size = guide_legend(override.aes = list(shape = c(21, 21, 21, 1), fill = c("black", "black", "black", "white")))) +
...
transition.outcome.list <- c("PD", "Dementia", "Mortality")
for (transition.outcome in transition.outcome.list){
  heatmap.sep <- heatmap(data = nfl_gfap_long_final, exposure = "Neurodegenerative markers", transition.outcome = transition.outcome)
  ggsave(paste(plot.out.folder, "Neurodegenerative markers_", transition.outcome, "_heatmap.jpeg",sep=""), heatmap.sep, width = 9, height = 10, units = "in", dpi = 300)
}

for (transition.outcome in transition.outcome.list){
  heatmap.sep <- heatmap(data = air_pollution_long_final, exposure = "Air pollution markers", transition.outcome = transition.outcome)
  ggsave(paste(plot.out.folder, "Air pollution markers_", transition.outcome, "_heatmap.jpeg",sep=""), heatmap.sep, width = 9, height = 10, units = "in", dpi = 300)
} 
